        ...
        <div class="content_right_box content_padding">
            ...
            <div class="list">
                ...
                <div class="line mark_a">
                    <router-link :to="'/symptom/survey/'+item.id" class="mark_a" v-for="(item,index) in popular" :key="item.id"  v-if="index ==popular.length-1">{{item.name}}</router-link>
                    <router-link :to="'/symptom/survey/'+item.id" class="mark_a" v-else>{{item.name}},</router-link>
                </div>
            </div>
            <div class="list">
                <h4>常用症状:</h4>
                <div class="line mark_a">
                        <router-link :to="'/symptom/survey/'+item.id" class="mark_a" v-for="(item,index) in commonUse" :key="item.id"  v-if="index ==popular.length-1">{{item.name}},</router-link>
                        <router-link :to="'/symptom/survey/'+item.id" class="mark_a" v-else>{{item.name}},</router-link>
                </div>
            </div>
            <div class="list">
